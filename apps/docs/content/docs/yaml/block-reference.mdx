...
Block references are the foundation of data flow in Agent Forge workflows. Understanding how to correctly reference outputs from one block as inputs to another is essential for building functional workflows.
...
Agent Forge validates all references when importing YAML:
