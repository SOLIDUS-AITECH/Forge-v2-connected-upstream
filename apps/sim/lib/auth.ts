  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url, token }, request) => {
      ...
      const result = await resend.emails.send({
        from: `Agent Forge <team@${getEmailDomain()}>`,
        to: user.email,
    ...
    emailOTP({
      sendVerificationOTP: async (data: {
        ...
        try {
          ...
          const result = await resend.emails.send({
            from: `Agent Forge <onboarding@${getEmailDomain()}>`,
            to: data.email,
            ...
            sendInvitationEmail: async (data: any) => {
              try {
                ...
                await resend.emails.send({
                  from: `Agent Forge <team@${getEmailDomain()}>`,
                  to: invitation.email,
                  subject: `${inviterName} has invited you to join ${organization.name} on Agent Forge`,
                  html,
                })
              } catch (error) {
