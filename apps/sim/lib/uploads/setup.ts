...
export const BLOB_EXECUTION_FILES_CONFIG = {
  ...
  accountKey: env.AZURE_ACCOUNT_KEY || '',
  connectionString: env.AZURE_CONNECTION_STRING || '',
  containerName: 'sim-execution-files',
}
 ...
 * Only needed when using local storage
 */
export async function ensureUploadsDirectory(): Promise<boolean> {
  if (USE_S3_STORAGE || USE_BLOB_STORAGE) {
    // Cloud storage is configured, no need for local directory
    return true
  }

  try {
    // For client-side, we can't create directories, so just return success
    // This function is mainly used server-side
    return true
  } catch (error) {
