import { pgTable, unique, text, boolean, timestamp, integer, numeric, date, foreignKey, json, index, uniqueIndex, jsonb, check, uuid, vector, pgEnum } from "drizzle-orm/pg-core"
...
export const bannedUsers = pgTable("banned_users", {
	...
	unique("banned_users_email_key").on(table.email),
]);

export const dailyUserStats20250815 = pgTable("daily_user_stats_20250815", {
	id: text(),
	userId: text("user_id"),
	...
	totalChatExecutions: integer("total_chat_executions"),
	currentUsageLimit: numeric("current_usage_limit"),
	usageLimitSetBy: text("usage_limit_set_by"),
	usageLimitUpdatedAt: timestamp("usage_limit_updated_at", { mode: 'string' }),
	currentPeriodCost: numeric("current_period_cost"),
	billingPeriodStart: timestamp("billing_period_start", { mode: 'string' }),
	billingPeriodEnd: timestamp("billing_period_end", { mode: 'string' }),
	lastPeriodCost: numeric("last_period_cost"),
	recordCreationDate: date("record_creation_date"),
});
...
export const userStatsDaily = pgTable("user_stats_daily", {
	id: text(),
	userId: text("user_id"),
	totalManualExecutions: integer("total_manual_executions"),
	totalApiCalls: integer("total_api_calls"),
...
export const embedding = pgTable("embedding", {
	...
	index("emb_doc_enabled_idx").using("btree", table.documentId.asc().nullsLast().op("bool_ops"), table.enabled.asc().nullsLast().op("text_ops")),
