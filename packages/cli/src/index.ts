...
program.name('simstudio').description('Run Agent Forge using Docker').version('0.1.0')

program
  .option('-p, --port <port>', 'Port to run Agent Forge on', DEFAULT_PORT)
...
async function main() {
  ...
  console.log(chalk.blue('🚀 Starting Agent Forge...'))
  ...
  if (!appSuccess) {
    console.error(chalk.red('❌ Failed to start Agent Forge'))
    process.exit(1)
  }

  console.log(
    chalk.green(`✅ Agent Forge is now running at ${chalk.bold(`http://localhost:${port}`)}`)
  )
  console.log(
    chalk.yellow(
  ...
  rl.on('SIGINT', async () => {
    ...
    console.log(chalk.green('✅ Agent Forge has been stopped'))
    process.exit(0)
  })
}
